...
SaberStatus Tensor<BM>::copy_from<X86>(const Tensor<X86>& tensor) {
    ...
    BMDNN_CHECK(bm_memcpy_s2d(GET_BM_HANDLE(0), *device_data_ptr, bm_mem_from_system(const_cast<float *>((float*) tensor.data()))));
    return SaberSuccess;
}
...
SaberStatus Tensor<X86>::copy_from<BM>(const Tensor<BM>& tensor) {
    ...
    BMDNN_CHECK(bm_memcpy_d2s(GET_BM_HANDLE(0), bm_mem_from_system((float*) mutable_data()), *device_data_ptr));
    return SaberSuccess;
}
#endif
#endif
